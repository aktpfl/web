...
<?php if($__Context->XE_VALIDATOR_MESSAGE && $__Context->XE_VALIDATOR_ID == 'modules/communication/tpl/index/1'){ ?><div class="message <?php echo escape($__Context->XE_VALIDATOR_MESSAGE_TYPE, false) ?>">
	...
	<div class="x_control-group">
		...
		<div class="x_controls">
			<select name="editor_skin" id="editor_skin" onchange="getEditorSkinColorList(this.value)">
				<?php if($__Context->editor_skin_list&&count($__Context->editor_skin_list))foreach($__Context->editor_skin_list as $__Context->editor_skin){ ?><option value="<?php echo escape($__Context->editor_skin, false) ?>"<?php if($__Context->editor_skin==$__Context->communication_config->editor_skin){ ?> selected="selected"<?php } ?>><?php echo escape($__Context->editor_skin, false) ?></option><?php } ?>
			</select>
			<select name="sel_editor_colorset" style="display:none">
			</select>
			<script>
			//<![CDATA[
				getEditorSkinColorList('<?php echo escape($__Context->communication_config->editor_skin, false) ?>','<?php echo escape($__Context->communication_config->editor_colorset, false) ?>');
			//]]>
			</script>
		</div>
	</div>
	<div class="x_control-group">
		...
		<div class="x_controls">
			<select id="layout" name="layout_srl" style="width:auto">
				...
				<?php if($__Context->layout_list&&count($__Context->layout_list))foreach($__Context->layout_list as $__Context->key=>$__Context->val){ ?><option value="<?php echo escape($__Context->val->layout_srl, false) ?>"<?php if($__Context->val->layout_srl == $__Context->communication_config->layout_srl){ ?> selected="selected"<?php } ?>><?php echo escape($__Context->val->title, false) ?> (<?php echo escape($__Context->val->layout, false) ?>)</option><?php } ?>
			</select>
		</div>
	</div>
	<div class="x_control-group">
		...
		<div class="x_controls">
			<select name="skin" id="skin" style="width:auto" onchange="doGetSkinColorset(this.options[this.selectedIndex].value, 'P')">
				<?php if($__Context->communication_skin_list&&count($__Context->communication_skin_list))foreach($__Context->communication_skin_list as $__Context->key=>$__Context->val){ ?><option value="<?php echo escape($__Context->key, false) ?>"<?php if($__Context->key==$__Context->communication_config->skin){ ?> selected="selected"<?php } ?>><?php echo escape($__Context->val->title, false) ?></option><?php } ?>
			</select>
		</div>
	</div>
	...
	<div class="x_control-group">
		...
		<div class="x_controls">
			<select id="mlayout_srl" name="mlayout_srl" style="width:auto">
				<option value="0"><?php echo $__Context->lang->notuse ?></option>
				<?php if($__Context->mlayout_list&&count($__Context->mlayout_list))foreach($__Context->mlayout_list as $__Context->key=>$__Context->val){ ?><option value="<?php echo escape($__Context->val->layout_srl, false) ?>"<?php if($__Context->val->layout_srl == $__Context->communication_config->mlayout_srl){ ?> selected="selected"<?php } ?>><?php echo escape($__Context->val->title, false) ?> (<?php echo escape($__Context->val->layout, false) ?>)</option><?php } ?>
			</select>
		</div>
	</div>
	<div class="x_control-group">
		...
		<div class="x_controls">
			<select name="mskin" id="mskin" style="width:auto" onchange="doGetSkinColorset(this.options[this.selectedIndex].value, 'M')">
				<?php if($__Context->communication_mobile_skin_list&&count($__Context->communication_mobile_skin_list))foreach($__Context->communication_mobile_skin_list as $__Context->key=>$__Context->val){ ?><option value="<?php echo escape($__Context->key, false) ?>"<?php if($__Context->key==$__Context->communication_config->mskin){ ?> selected="selected"<?php } ?> ><?php echo escape($__Context->val->title, false) ?></option><?php } ?>
			</select>
		</div>
	</div>
	...
	<div class="x_control-group">
		...
		<div class="x_controls">
			...
			<div id="zone_grant_write" hidden style="margin:8px 0 0 0">
				<?php if($__Context->group_list&&count($__Context->group_list))foreach($__Context->group_list as $__Context->group_srl=>$__Context->group_item){ ?><label for="grant_write_group_<?php echo escape($__Context->group_srl, false) ?>">
					<input type="checkbox" class="checkbox" name="grant_write_group[]" value="<?php echo escape($__Context->group_item->group_srl, false) ?>" id="grant_write_group_<?php echo escape($__Context->group_item->group_srl, false) ?>"<?php if(isset($__Context->communication_config->grant_write['group_grant'][$__Context->group_srl])&&$__Context->communication_config->grant_write['group_grant'][$__Context->group_srl]==$__Context->group_item->title){ ?> checked="checked"<?php } ?>/>
...
<script>
    jQuery(function() { 
		doGetSkinColorset("<?php echo escape($__Context->communication_config->skin, false) ?>", 'P'); 
		doGetSkinColorset("<?php echo escape($__Context->communication_config->mskin, false) ?>", 'M'); 
